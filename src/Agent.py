...
class Agent(nn.Module):
    def __init__(self, option, data_loader, graph=None):
        ...
        self.graph = graph
        self.relation_embedding = nn.Embedding(self.option.num_relation+self.option.num_entity, self.option.relation_embed_size)
    ...
    def step(self, prev_state, prev_relation, current_entities, start_entities, queries, answers, all_correct, step):
        ...
        dummy_relations_id = torch.ones_like(out_relations_id, dtype=torch.int64) * self.data_loader.kg.pad_token_id
